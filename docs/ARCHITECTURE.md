...
| Поиск подписок | Кластеризация, признаки периодичности, классификация через ChatGPT 5.6 Terra | Не реализован |
...
flowchart TD
    ...
    Detect --> LLM[Адаптер ChatGPT 5.6 Terra]
...
│   │   │   └── tbank_pdf.py
│   │   ├── llm/
│   │   │   ├── ChatGPT 5.6 Terra.py           # HTTP, таймауты, повторы
...
Это целевая структура: перечисленные новые модули ещё предстоит реализовать. `domain` не импортирует Telegram, FastAPI, SQLAlchemy или ChatGPT 5.6 Terra. `application` использует доменные функции и контракты; инфраструктура реализует эти контракты. Сборка зависимостей происходит в точках запуска.
...
| demo/integrations/ChatGPT 5.6 Terraai.py и llm_helpers.py | Единый адаптер с ограниченными таймаутами, повтором временных ошибок и проверкой `success` и схемы |
